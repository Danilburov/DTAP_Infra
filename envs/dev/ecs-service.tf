...
resource "aws_ecs_service" "dtap-backend-prod" {
  name = "dtap-backend-prod"
  cluster = aws_ecs_cluster.dtap-cluster.arn
  task_definition = aws_ecs_task_definition.iac-dtap-backend-prod.arn
  desired_count = 1
  launch_type = "FARGATE"
...
resource "aws_ecs_service" "dtap-frontend-prod" {
  name = "dtap-frontend-prod"
  cluster = aws_ecs_cluster.dtap-cluster.arn
  task_definition = aws_ecs_task_definition.iac-dtap-frontend-prod.arn
  desired_count = 1
  launch_type = "FARGATE"
